...
Abaqus 作业监控工具 - 监控作业状态，支持飞书/企业微信通知和 CSV 记录
...
- 检测孤立作业（进程异常终止但 .lck 文件未删除）
- 支持动态添加/移除监控目录（无需重启服务）
- 进度条显示和 .sta 文件解析
- 从 .inp 文件解析总分析步时间，计算完成百分比
...
| `CSV_FILENAME` | `abaqus_jobs_%Y%m.csv` | 文件名模板，支持日期格式化和 `{folder}` 占位符 |
| `CSV_UPDATE_INTERVAL` | `60` | 定时更新间隔（秒），0 表示仅在完成时更新 |
| `CSV_OVERWRITE_MODE` | `none` | 覆盖模式：`none`/`running`/`always` |
| `CSV_MAX_HISTORY` | `5` | 同一作业保留历史记录数，0 表示不限制 |
...
| `PROGRESS_NOTIFY_INTERVAL` | `3600` | 进度通知间隔（秒），0 表示禁用 |
| `ENABLE_PROCESS_DETECTION` | `true` | 是否启用进程检测（检测孤立作业） |
...
### Q: 如何只使用 CSV 记录，不发送通知？

A: 将 `FEISHU_WEBHOOK_URL` 和 `WECOM_WEBHOOK_URL` 留空即可：
```env
FEISHU_WEBHOOK_URL=
WECOM_WEBHOOK_URL=
```

### Q: CSV 文件中出现很多重复记录怎么办？

A: 设置覆盖模式为 `running`，调试时会自动覆盖未完成的记录：
```env
CSV_OVERWRITE_MODE=running
CSV_MAX_HISTORY=5
```

### Q: 作业异常终止后显示"孤立作业"是什么意思？

A: 当 Abaqus 进程已停止，但 `.lck` 文件仍然存在时，会被判定为孤立作业。这通常发生在：
- 手动终止 Abaqus 进程
- 系统崩溃或断电
- Abaqus 异常退出
...
A: 直接编辑 `.env` 文件中的 `WATCH_DIRS` 并保存，程序会在下一个轮询周期（默认 5 秒）自动检测变化并生效。

### Q: 进度百分比显示为空是什么原因？

A: 进度百分比需要从 `.inp` 文件中解析总分析步时间。如果 `.inp` 文件不存在或格式不支持，则无法计算百分比。
